...
### 2.3. Các Mô Hình Lai (Hybrid Models)
3. **Mô hình Lai Cơ bản (Hybrid SVD + CB):** Phương pháp kết hợp điểm số (Late Fusion) giữa dự đoán của Matrix Factorization (SVD) và độ tương đồng Cosine của Content-Based.
4. **Mô hình Đề xuất (Advanced Hybrid GNN):** Sử dụng Mạng nơ-ron đồ thị đa loại (Heterogeneous GNN) hoạt động trên Đồ thị Tri thức Cộng tác (Collaborative Knowledge Graph - CKG), kết hợp kiến trúc học biểu diễn tiên tiến nhất. Hệ thống xây dựng một đồ thị mà ở đó các đỉnh bao gồm cả Người dùng, Món ăn, Nguyên liệu, và Đặc tính. Các cạnh biểu diễn các mối quan hệ đa dạng (ví dụ: User-tương tác-Item, Item-chứa-Ingredient).
...
| Độ đo (Metric) | Lọc Nội Dung (CB) | Lọc Cộng Tác (MF/SVD) | Lọc Lai Cơ Bản (Hybrid SVD+CB) | GNN (Đề xuất / Advanced Hybrid) | Đánh giá tổng quan |
| :--- | :---: | :---: | :---: | :---: | :--- |
| **Recall@20** | 0.0865 | 0.0460 | 0.0721 | 0.0600 | GNN cân bằng, cải thiện tốt hơn MF |
| **Precision@20**| 0.0641 | 0.0401 | 0.0554 | 0.0480 | CB cao do quá thiên vị lịch sử hẹp |
| **NDCG@20** | 0.1683 | 0.0578 | 0.1145 | 0.0766 | GNN cải thiện 32.5% so với MF |
| **MRR@20** | 0.5076 | 0.1356 | 0.3120 | 0.1800 | GNN cải thiện 32.7% so với MF |
| **Hit Rate@20** | 0.5890 | 0.4056 | 0.4980 | 0.4839 | GNN cải thiện 19.3% so với MF |
| **Coverage@20** | 0.1066 | 0.1243 | 0.1520 | **0.5497** | **GNN vượt trội hoàn toàn (Gấp ~4-5 lần các mô hình khác)** |
...
### 4.3. Giới Hạn Của Mô Hình Lọc Lai Cơ Bản (Hybrid SVD + CB)
- **Lý thuyết:** Bằng cách kết hợp dự đoán (Late Fusion) theo công thức `Score = α * SVD + (1-α) * CB`, mô hình Hybrid Cơ bản cố gắng lấy độ phủ của SVD đắp vào độ chính xác của Content-Based.
- **Vấn đề thực tiễn:** Bảng kết quả cho thấy Hybrid SVD+CB có sự cải thiện so với SVD đơn thuần. Tuy nhiên, chỉ số **Coverage vẫn lẹt đẹt ở mức 0.1520**. Việc cộng điểm số một cách tuyến tính chỉ là phương pháp gượng ép cơ học, không thực sự tạo ra các liên kết ẩn sâu sắc giữa các món ăn. Nó vẫn không giải quyết triệt để bài toán Khám phá (Serendipity) cho người dùng.

### 4.4. GNN - Kiến Trúc Lai Toàn Diện (The Advanced Hybrid Solution)
Việc ứng dụng GNN trên nền tảng Đồ thị Tri thức Cộng tác (CKG) chính là chìa khóa giải quyết đồng thời các nghịch lý của hệ tư vấn truyền thống:

- **Tính Khám Phá Tuyệt Đối (Coverage = 0.5497):** GNN gây ấn tượng mạnh mẽ với độ phủ xấp xỉ 55%, **vượt xa hoàn toàn (gấp 4-5 lần)** so với các mô hình cơ sở và mô hình Lai tuyến tính. Bằng cách kết nối đồ thị dựa trên các thuộc tính (Nguyên liệu, Thể loại) và cả hành vi mua sắm, cơ chế Message Passing của GNN có thể liên kết những món ăn tưởng chừng như khác biệt về mặt ngữ nghĩa, nhưng lại có mối liên hệ thông qua hành vi của một nhóm người dùng. Quá trình lan truyền đa chặng (multi-hop reasoning) này phá vỡ hoàn toàn bong bóng lọc.
- **Bảo Toàn Hiệu Năng Dự Đoán:** Không giống như MF bị sụp đổ hiệu năng khi cố gắng tăng Coverage, GNN tận dụng lượng thông tin phong phú từ cấu trúc đồ thị để cải thiện độ chính xác, mang lại hiệu năng cao hơn đáng kể so với thuật toán Collaborative Filtering cốt lõi. Khả năng xử lý vấn đề Dữ liệu thưa (Sparsity) và Khởi động lạnh (Cold-start) được giải quyết triệt để.
